...
{
    e_Difficulty Difficulte; // current game difficulty
    int Level; // current level
    int Lives; // current number of life
    int Score; // current score
    double Vitesse; // speed ​​depending on level
    double VitesseMoy; // current speed of the locomotive
    float EcartWagon; // size in pixels between wagons
    int NiveauMax;
    int FullScreen;
    int Langue; // locale index
    int NLangues;
...
struct sNewPreference
{
    ...
    float Volume { (float)0 }; // audio volume
    float VolumeM { (float)0 }; // music volume
    struct sScore Sco[8]; // store scores
